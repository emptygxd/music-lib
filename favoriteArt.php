...
$favArtistsQuery = $mysql -> query ("SELECT * FROM `favArtists` WHERE `userId` = '$id' ORDER BY `dateAdded` DESC;");
	...
	<section class="main-content" >
				...
				<div class="selections">
					<div class="podborki"> 
						...
						<p style="margin-left: 20px; margin-top: 20px">Список избранных исполнителей:</p>

					</div> 
					<div class="items">
						<?php  			
						while ($favArtistsInfo = $favArtistsQuery -> fetch_assoc()) {
                            $artistQuery = $mysql -> query ("SELECT * FROM `artist` WHERE `artistId` = '$favArtistsInfo[artistId]'");
                            $artistInfo = $artistQuery -> fetch_assoc();
